...
export default function LoginPage() {
  ...
  const { isConfigured, signInWithPassword, signUpWithPassword, signInWithGoogle } = useAuth();
  ...
  useEffect(() => {
    ...
    if (error === "auth_callback_failed") {
      ...
      setMessage("Server auth is not configured. Check Vercel env vars and redeploy.");
    }
  }, []);
      ...
      <main className="flex min-h-screen items-center justify-center p-6">
        <div className="max-w-md space-y-3 text-center">
          ...
          <p className="text-sm text-muted-foreground">
            Add your Supabase URL and anon key to <code className="text-xs">.env.local</code>, then
    ...
    <main className="flex min-h-screen items-center justify-center bg-background p-6">
      <div className="w-full max-w-sm space-y-6 rounded-xl border border-border bg-card p-6 shadow-sm">
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground">
            Save your protocols and settings across devices.
          </p>
        </div>
