...
/** CSS transition durations (ms) mapped from spring tiers for non-motion transitions. */
export const springCss = {
  fast: { enter: 80, exit: 60 },
  moderate: { enter: 160, exit: 120 },
  slow: { enter: 240, exit: 160 },
} as const;

export function springOpacityTransition(
  visible: boolean,
  tier: keyof typeof springCss = "moderate",
): string {
  const ms = visible ? springCss[tier].enter : springCss[tier].exit;
  return `opacity ${ms}ms ease`;
}
